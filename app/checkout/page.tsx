...
export default function CheckoutPage() {
    ...
    const [payMethod, setPayMethod] = useState<"mpesa" | "cod">("mpesa");
    ...
    const waMsg = encodeURIComponent(
        ...
        `\n\nTotal: ${formatKES(total)}\nDelivery to: ${address}, ${city}, ${county}\nPayment: ${payMethod === "mpesa" ? "M-Pesa" : "Cash on Delivery"}`
    );
                        ...
                        <div className="bg-white dark:bg-[#171717] rounded-2xl border border-zinc-200 dark:border-zinc-800 p-6 space-y-5">
                            ...
                            <div className="space-y-3">
                                ...
                                <label className={`flex items-start gap-4 p-4 rounded-xl border-2 cursor-pointer transition-all ${
                                    ...
                                    <input type="radio" name="payment" value="mpesa" checked={payMethod === "mpesa"} onChange={() => setPayMethod("mpesa")} className="mt-0.5 accent-[#C6A16A]" />
                                    <div className="flex-1">
                                        <p className="text-sm font-bold text-zinc-900 dark:text-white">M-Pesa</p>
                                        <p className="text-xs text-zinc-500 dark:text-zinc-400 mt-0.5">Pay via M-Pesa. You will receive payment instructions after placing your order.</p>
                                        {payMethod === "mpesa" && (
                                            <div className="mt-4">
                                                <Field
                                                    label="M-Pesa transaction reference (optional)"
                                                    value={mpesaRef} onChange={setMpesaRef}
                                                    placeholder="e.g. QGH3X7YZ89"
                                                    icon={<Phone className="w-4 h-4" />}
                                                    required={false}
                                                />
                                                <p className="text-[11px] text-zinc-400 mt-2 leading-relaxed">
                                                    You can enter the ref now if you have already paid, or leave it blank and provide it on WhatsApp after ordering.
                                                </p>
                                            </div>
                                        )}
                                    </div>
                                ...
                                <label className={`flex items-start gap-4 p-4 rounded-xl border-2 cursor-pointer transition-all ${
                                    ...
                                    <input type="radio" name="payment" value="cod" checked={payMethod === "cod"} onChange={() => setPayMethod("cod")} className="mt-0.5 accent-[#C6A16A]" />
                                    <div>
                            ...
                            <div className="flex items-center gap-2 px-4 py-3 rounded-xl bg-zinc-50 dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 text-xs text-zinc-500 dark:text-zinc-400">
                                ...
                                Your payment details are safe. We do not store card numbers or M-Pesa PINs.
                            </div>
                        </div>
                    )}
                        ...
                        <div className="space-y-4">
                            ...
                            <div className="bg-white dark:bg-[#171717] rounded-2xl border border-zinc-200 dark:border-zinc-800 p-5">
                                ...
                                <p className="text-sm font-semibold text-zinc-700 dark:text-zinc-300">
                                    {payMethod === "mpesa" ? "M-Pesa" : "Cash on Delivery"}
                                </p>
                                {mpesaRef && <p className="text-xs text-zinc-400 mt-1">Ref: {mpesaRef}</p>}
                            </div>
                        </div>
                    )}
