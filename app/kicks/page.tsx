...
function KickCard({ product }: { product: Product }) {
    ...
    return (
        <article className="group flex flex-col bg-white dark:bg-[#171717] rounded-2xl border border-zinc-200 dark:border-zinc-800 overflow-hidden shadow-sm hover:shadow-lg hover:border-[#C6A16A]/40 dark:hover:border-[#C6A16A]/30 transition-all duration-300">
            ...
            <div className="relative w-full aspect-square bg-zinc-100 dark:bg-zinc-900 overflow-hidden">
                ...
                <button type="button" onClick={handleWish} disabled={togglingWish}
                    className="absolute top-3 right-3 p-2 rounded-full bg-white/90 dark:bg-zinc-900/90 border border-zinc-200 dark:border-zinc-700 shadow-sm opacity-0 group-hover:opacity-100 transition-all duration-200 hover:scale-110 disabled:cursor-not-allowed"
            ...
            <div className="flex flex-col flex-1 p-4 gap-2">
                ...
                <span className="text-base font-bold text-zinc-900 dark:text-white">{formatKES(product.price)}</span>
        ...
        <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
            ...
            <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4 mb-10">
                <div>
                    ...
                    <h1 className="text-3xl sm:text-4xl font-mulish font-black text-zinc-900 dark:text-white leading-tight">
                        Castra Kicks
                    </h1>
            ...
            {loading ? (
                <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4 sm:gap-5">
                    {Array.from({ length: PER_PAGE }).map((_, i) => (
                        <div key={i} className="rounded-2xl bg-zinc-100 dark:bg-zinc-900 aspect-square animate-pulse" />
                    ))}
                </div>
            ) : products.length === 0 ? (
            ...
            {totalPages > 1 && !loading && (
                <div className="flex items-center justify-center gap-2 mt-12">
                    <button type="button" onClick={() => goTo(currentPage - 1)} disabled={currentPage === 1}
                        className="p-2.5 rounded-xl border border-zinc-200 dark:border-zinc-800 text-zinc-500 dark:text-zinc-400 hover:border-[#C6A16A]/50 hover:text-[#C6A16A] disabled:opacity-30 disabled:cursor-not-allowed transition-all">
